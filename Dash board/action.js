...
const generateDlRandom = ()=>{
	const dl = document.getElementById('dl');
	let dl_id = 'AD2019' + (parseInt(Math.random()*100 ));
	dl.value=dl_id;
		...
		<div class="card m-2 ${classProp}">
			...
			<div class="card-body">
				<div class="row">
					...
					<div class="col-md-8">
						<div class="${list.id}">
							<label for="name">Category of Ad:&nbsp;</label><span id="card_name"><strong>${list.name}</strong></span><br>
							<label for="dob">Date of Publishing:&nbsp;</label><span id="card_dob"><strong>${list.dob}</strong></span><br>
							<label for="address">Description:&nbsp;</label><span id="card_add"><strong>${list.address}</strong></span><br>
							<label for="dlno">AD.No.&nbsp;</label><span id="card_id"><strong>${list.id}</span></strong><br>
